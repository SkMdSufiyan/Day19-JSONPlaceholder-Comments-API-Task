...
page_heading.innerHTML = "Welcome to the Comments page";
...
document.getElementById('serach-instruction-div').innerHTML = "To search the comments from a particular Email Id, type the Email Id (without initial or extra spaces) in the search bar, it will highlight (it can not navigate, so after clicking search button kindly scroll down) that particular Email Id in the following list";
...
search_field.setAttribute('placeholder',"Email")
    ...
    try{
        ...
        for (let cell_count=0;cell_count<dataFromUrl.length;cell_count+=cells_per_page){
            ...
            if(dataFromUrl.length - cell_count>=cells_per_page){
                // If the number of comments still remaining is greater than or equal to the number of cells per page
                num_of_cells_available = cell_count+cells_per_page;
            }else{
                 // If the number of comments still remaining is less than the number of cells per page
            ...
            for(let i=cell_count;i<num_of_cells_available;i+=cells_per_row){
                ...
                for(let j=0; j<cells_per_row; j++){
                    if(i+j<dataFromUrl.length){
                        ...
                        comment_content_div.classList.add('comment-content-div');
                        name_div.classList.add('name-div');
                        id_div.classList.add('id-div');
                        email_div.classList.add('email-div');
        
        
                        comment_content_div.classList.add('container');
                        name_div.classList.add('container');
                        id_div.classList.add('container');
                        email_div.classList.add('container');
        
                        
                        comment_content_div.innerHTML = `<strong>${ith_data["body"]}</strong>`;
                        name_div.innerHTML = '<span>Name: </span>' + ith_data['name'];
                        id_div.innerHTML = '<span>Id: </span>' + ith_data['id'];
                        email_div.innerHTML = '<span>Email: </span>' + ith_data['email'];
                        ...
                        // Appending the comment, name, id, and email to table cell
                        table_cell.append(comment_content_div);
                        table_cell.append(name_div);
                        table_cell.append(id_div);
                        table_cell.append(email_div);
...
function markingFun(){   
    ...
    if(search_input.value!=''){
        ...
        let emails =document.querySelectorAll('.email-div'); 
        
        let previous_backgroundColors = [];
        emails.forEach((email_id)=>{
            
            if(email_id.innerHTML.toLowerCase().includes(search_value_lowerCase)){
                previous_backgroundColors.push(email_id.style.backgroundColor);
                email_id.style.backgroundColor = 'yellow';
            }
        })
...
function unmarkingFun(){ 
    if(previous_searched_memory!=null){
            ...
            emails.forEach((email_id)=>{
                if(email_id.innerHTML.toLowerCase().includes(prev_search_value_lowerCase)){
                    email_id.style.backgroundColor = originalBackColors[original_backcolor_index];
